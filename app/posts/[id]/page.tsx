...
export default function PostDetailPage() {
  ...
  useEffect(() => {
    // 실제 애플리케이션에서는 이 로직이 백엔드에서 처리되어야 합니다.
    // (예: 게시글을 가져오는 API가 호출될 때 조회수를 1 증가시킴)
    // 여기서는 프론트엔드에서 조회수 증가를 시뮬레이션합니다.
    const incrementViewCount = (post: Post): Post => {
      return {
        ...post,
        views: post.views + 1,
      };
    ...
    setPost(incrementViewCount(mockPost));
  ...
  const sortedComments = useMemo(() => {
    ...
    // 베스트 댓글 기능 활성화 조건
    const isBestCommentFeatureActive = comments.length >= 10;
    ...
    const potentialBest = comments.filter((c) => c.likes >= 5);
    potentialBest.sort((a, b) => {
      if (a.likes !== b.likes) return b.likes - a.likes; // 좋아요 많은 순
      const dateDiff = b.createdAt.getTime() - a.createdAt.getTime(); // 최신순
      if (dateDiff !== 0) return dateDiff;
      return a.id.localeCompare(b.id); // 고유 ID로 최종 정렬
    });

    const bestComments = potentialBest.slice(0, 3);
    const bestCommentIds = new Set(bestComments.map((c) => c.id));

    const otherComments = comments.filter((c) => !bestCommentIds.has(c.id));
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="space-y-3">
        ...
        {sortedComments.map((comment, index) => {
          ...
          const isTopComment =
            comments.length >= 10 && index < 3 && comment.likes >= 5;
          return (
            <Card
              key={comment.id}
