...
export default function ProfilePage() {
  ...
  // ✅ 해결: 의존성 배열을 []로 비워서 초기 1회만 실행되도록 수정
  useEffect(() => {
    if (user) {
      setName(user.name || "");
          ...
          <CardContent>
            <Tabs defaultValue="posts">
              <TabsList className="grid w-full grid-cols-2 bg-slate-100/80 p-1 rounded-lg h-auto">
                <TabsTrigger
                  value="posts"
                  className="py-2.5 data-[state=active]:bg-white data-[state=active]:text-blue-600 data-[state=active]:shadow-md font-semibold transition-all duration-300"
                >
                          ...
                          <CardContent className="p-5 flex h-40 items-center gap-5">
                            ...
                            <div className="py-1 flex flex-col flex-1 min-w-0 h-full">
                              <div>
                                ...
                                <h4 className="font-semibold text-lg truncate mt-1 group-hover:text-blue-600 transition-colors">
                                  {post.title}
                                </h4>
