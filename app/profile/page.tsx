...
import { Edit, Save, Heart, Eye, FileText, MessageSquare, Image as ImageIcon, Pencil, PlusCircle } from "lucide-react";
          ...
          {isLoadingPosts ? (
            ...
            myPosts.map((post) => (
              <Link href={`/posts/${post.postId}`} key={post.postId}>
                <Card className="overflow-hidden transition-all duration-300 ease-in-out hover:shadow-xl hover:-translate-y-1 group border-transparent hover:border-blue-500">
                  <CardContent className="p-0 flex h-32">
                    {post.photo ? (
                      <div className="w-32 flex-shrink-0 relative">
                        <img src={`${API_BASE}${post.photo}`} alt={post.title} className="w-full h-full object-cover" />
                      </div>
                    ) : (
                    ...
                    <div className="p-4 flex flex-col justify-between flex-1 min-w-0">
                      <div>
                        ...
                        <h4 className="font-bold text-lg truncate mt-1 group-hover:text-blue-600 transition-colors">{post.title}</h4>
                        <p className="text-sm text-gray-500 mt-1 line-clamp-1">
                          {post.content}
                        </p>
                      </div>
                      <div className="text-xs text-gray-400 mt-2 flex items-center justify-between">
                        <span>{formatRelativeTime(post.createdDate)}</span>
          ...
          {isLoadingComments ? (
            ...
            myComments.map((comment) => (
              <Link href={`/posts/${comment.postId}#comment-${comment.commentId}`} key={comment.commentId}>
                <Card className="transition-all duration-300 ease-in-out hover:shadow-lg hover:border-blue-400">
                  <CardContent className="p-4">
                    ...
                    <div className="mt-3 flex justify-between items-center text-sm">
                      <p className="text-gray-500 truncate">
                        <span className="font-normal">on:</span> <span className="font-semibold text-gray-600 hover:underline">{comment.postTitle}</span>
                      </p>
                      <div className="flex items-center gap-4 text-xs text-gray-400 flex-shrink-0 ml-4">
                        ...
                        <span className="flex items-center gap-1"><Heart className="h-3 w-3 text-red-400" /> {comment.likeCount}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
