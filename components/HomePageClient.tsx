...
function HomePageClientContent() {
  ...
  // 검색어 입력 시 URL 파라미터를 'keyword'로 변경
  useEffect(() => {
    if (keyword !== debouncedSearchKeyword) {
      const params = new URLSearchParams(searchParams.toString());
      if (debouncedSearchKeyword) {
        params.set('keyword', debouncedSearchKeyword);
      } else {
  ...
  }, [debouncedSearchKeyword, keyword, pathname, router, searchParams]);
